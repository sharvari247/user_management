...
import { INSERT_USER, FIND_USER_BY_EMAIL, FIND_USER_BY_ID, GET_ALL_USERS, UPDATE_USER, DELETE_USER, } from "./users.queries";
...
export const getCurrentUser = async (params: { user_id: string }) => {
...
export const getUserById = async (params: { user_id: string }) => {
    const { user_id } = params;
...
export const deleteUser = async (params: { user_id: string }) => {
    const { user_id } = params;
